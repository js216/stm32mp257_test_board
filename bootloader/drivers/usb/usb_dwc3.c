...
/*
 * Bare-metal carry addition: open/configure a non-control endpoint. The
 * TF-A DFU use-case only ever exercised EP0, so the driver has no public
 * endpoint-open; this mirrors the EP0 DEPCFG sequence for class endpoints
 * (bulk/interrupt) and enables them in DALEPENA. TRBs for these endpoints
 * are carved from the same coherent page, after the EP0 areas
 * (2048-byte event buffer + 2 TRBs + setup packet), at a 64-byte-aligned
 * offset, one slot per physical endpoint.
 */
enum usb_status usb_dwc3_ep_open(dwc3_handle_t *dwc3_handle, uint8_t ep_addr,
				 uint8_t type, uint16_t maxpacket)
{
	uint8_t num = ep_addr & EP_NUM_MASK;
	bool is_in = (ep_addr & EP_DIR_IN) != 0U;
	usb_dwc3_endpoint_t *dwc3_ep = is_in ? &dwc3_handle->IN_ep[num] :
					       &dwc3_handle->OUT_ep[num];
	struct usbd_ep *ep = is_in ? &dwc3_handle->pcd_handle->in_ep[num] :
				     &dwc3_handle->pcd_handle->out_ep[num];
	uintptr_t trb_area = (STM32MP_USB_DWC3_BASE + USB_DWC3_EVENT_BUFFER_SIZE +
			      (2U * sizeof(usb_dwc3_trb_t)) + 64U + 63U) & ~63UL;
	uintptr_t trb_slot = trb_area + ((uintptr_t)dwc3_ep->phy_epnum *
	...
	dwc3_ep->trb_flag = USB_DWC3_TRBCTL_NORMAL;
	dwc3_ep->trb_addr = (usb_dwc3_trb_t *)trb_slot;
	dwc3_ep->trb_dma_addr = (uint32_t)trb_slot;

	ret = dwc3_ep_configure(dwc3_handle, num, is_in, type, maxpacket,
				is_in ? num : 0U, 0U, dwc3_ep->phy_epnum,
				dwc3_ep->intr_num, USB_DWC3_DEPCFG_ACTION_INIT);
	if (ret != USBD_OK) {
		return ret;
	}

	DWC3_regupdateset(dwc3_handle->usb_device, DWC3_DALEPENA,
			  DWC3_DALEPENA_EP(dwc3_ep->phy_epnum));
