...
#define SDMMC_STAR_TXFIFOHE		BIT(14)
...
static int stm32_sdmmc2_send_cmd_req(struct mmc_cmd *cmd)
{
	...
	case MMC_CMD(12):
		...
		break;
	case MMC_CMD(24): /* bare-metal carry: block write support */
	case MMC_CMD(25):
		cmd_reg |= SDMMC_CMDR_CMDTRANS;
		break;
	case MMC_CMD(17):
...
/* Bare-metal carry: the mmc framework calls prepare() for both directions
 * with no hint; sd_glue flags an upcoming write so the data path direction
 * is programmed correctly (DTDIR set = card-to-host). */
static bool sdmmc2_write_pending;

void stm32_sdmmc2_set_write_pending(bool w)
{
	sdmmc2_write_pending = w;
}
...
static int stm32_sdmmc2_prepare(int lba, uintptr_t buf, size_t size)
{
	...
	int ret;
	uintptr_t base = sdmmc2_params.reg_base;
	uint32_t data_ctrl = sdmmc2_write_pending ? 0U : SDMMC_DCTRLR_DTDIR;
	uint32_t arg_size;
...
/* Bare-metal carry: polled block write (TF-A leaves this a stub). */
static int stm32_sdmmc2_write(int lba, uintptr_t buf, size_t size)
{
	uint32_t error_flags = SDMMC_STAR_TXUNDERR | SDMMC_STAR_DCRCFAIL |
			       SDMMC_STAR_DTIMEOUT;
	uint32_t flags = error_flags | SDMMC_STAR_DATAEND;
	uint32_t status;
	...
	do {
		...
		if ((remaining != 0U) &&
		    ((status & SDMMC_STAR_TXFIFOHE) != 0U)) {
			uint32_t count;

			/* Half-empty: room for SDMMC_FIFO_SIZE/2 bytes. */
			for (count = 0; (count < (SDMMC_FIFO_SIZE / 2U)) &&
			     (remaining != 0U); count += sizeof(uint32_t)) {
				mmio_write_32(fifo_reg, *buffer);
				buffer++;
...
/*
 * Bare-metal carry: the device-tree probe is replaced by the fixed DK
 * configuration (stm32mp257f-dk.dts &sdmmc1: bus-width 4, st,neg-edge,
 * vmmc handled by the PMIC rails already switched on at boot). Clock and
 * reset ids are unused -- the compat clk/reset shims act on SDMMC1
 * directly.
 */
static int stm32_sdmmc2_dt_get_config(void)
{
	sdmmc2_params.clock_id = 0U;
	sdmmc2_params.reset_id = 0U;
	sdmmc2_params.negedge = SDMMC_CLKCR_NEGEDGE;
	sdmmc2_params.bus_width = MMC_BUS_WIDTH_4;
	sdmmc2_params.max_freq = 25000000U;
